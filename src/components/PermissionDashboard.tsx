...
import { usePermissionSync } from './PermissionSync';
...
const PermissionDashboard: React.FC<PermissionDashboardProps> = ({
  ...
  const { syncPermissions, isSyncing, lastSync, syncError, permissionCount } = usePermissionSync();
    ...
    <div className={`permission-dashboard ${className}`} style={{ 
      ...
      {(syncError || lastSync) && (
        <div style={{
          ...
          borderRadius: '4px',
          marginBottom: '15px',
          backgroundColor: syncError ? '#ffebee' : '#e8f5e8',
          border: `1px solid ${syncError ? '#ffcdd2' : '#c8e6c8'}`,
          fontSize: '14px'
        }}>
          {syncError ? (
