...
class WebSocketService {
 ...
 connectToVenue(venueId: string, token: string, handlers: WebSocketEventHandlers = {}) {
  ...
  // Clean the token - remove any quotes or extra formatting
  ...
  // Add venue and token parameters

  const separator = wsUrl.includes('?') ? '&' : '?';

  const finalUrl = `${wsUrl}/venue/${venueId}${separator}token=${encodeURIComponent(cleanToken)}`;



  console.log('🔌 Connecting to WebSocket:', finalUrl.replace(cleanToken, '[TOKEN]'));
 ...
 connectToUser(userId: string, token: string, handlers: WebSocketEventHandlers = {}) {
  ...
  // Add user and token parameters

  const separator = wsUrl.includes('?') ? '&' : '?';

  const finalUrl = `${wsUrl}/user/${userId}${separator}token=${encodeURIComponent(cleanToken)}`;



  console.log('🔌 Connecting to User WebSocket:', finalUrl.replace(cleanToken, '[TOKEN]'));
